...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Compresser des fichiers en ZIP
    if ($action === 'compress') {
        $files = json_decode($_POST['files'] ?? '[]', true);
        $archiveName = trim($_POST['archive_name'] ?? '');
        
        if (empty($files) || empty($archiveName)) {
            header('Location: panel.php?path=' . urlencode($currentPath) . '&error=no_selection');
            exit;
        }
        
        // S'assurer que le nom se termine par .zip
        if (!preg_match('/\.zip$/i', $archiveName)) {
            $archiveName .= '.zip';
        }
        
        $archivePath = securePath($CLOUD_STORAGE_DIR, $currentPath . '/' . $archiveName);
        
        if (!class_exists('ZipArchive')) {
            header('Location: panel.php?path=' . urlencode($currentPath) . '&error=zip_not_available');
            exit;
        }
        
        $zip = new ZipArchive();
        if ($zip->open($archivePath, ZipArchive::CREATE) !== TRUE) {
            header('Location: panel.php?path=' . urlencode($currentPath) . '&error=zip_failed');
            exit;
        }
        ...
        header('Location: panel.php?path=' . urlencode($currentPath) . '&success=compressed');
        exit;
    }
    ...
    if ($action === 'extract') {
        ...
        if ($zip->open($fullPath) !== TRUE) {
            header('Location: panel.php?path=' . urlencode($currentPath) . '&error=extract_failed');
            exit;
        }
        
        // Créer un dossier pour l'extraction
        $extractFolderName = pathinfo($fullPath, PATHINFO_FILENAME);
        $extractPath = dirname($fullPath) . '/' . $extractFolderName;
        
        // Gérer les doublons
        $counter = 1;
        while (file_exists($extractPath)) {
            $extractPath = dirname($fullPath) . '/' . $extractFolderName . '_' . $counter;
            $counter++;
        }
        ...
        header('Location: panel.php?path=' . urlencode($currentPath) . '&success=extracted');
        exit;
    }
}
