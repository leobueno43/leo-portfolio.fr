<?php
// api/order_actions.php - Actions commandes
// ============================================
...
switch ($action) {
    ...
    case 'add_tracking':
        ...
        if ($order_id <= 0 || empty($tracking_number)) {
            echo json_encode(['success' => false, 'message' => 'Données invalides']);
            exit;
        }
        
        try {
            $stmt = $pdo->prepare("UPDATE orders SET tracking_number = ?, status = 'shipped', shipped_at = NOW() WHERE id = ?");
            if ($stmt->execute([$tracking_number, $order_id])) {
                echo json_encode(['success' => true, 'message' => 'Numéro de suivi ajouté']);
            } else {
                echo json_encode(['success' => false, 'message' => 'Erreur de mise à jour']);
            }
        } catch (PDOException $e) {
